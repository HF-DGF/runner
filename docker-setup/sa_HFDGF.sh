...
function build_with_HFDGF() {
    ...
    for TARG in "${@:2}"; do
        ...
        # save the time to the end of file sa_time.txt
        echo "HFDGF,$1,$BIN_NAME,$step1_start_time,$step1_end_time,$step1_duration" >> /sa_time_bin.txt
        ...
        for BUG_NAME in "${str_array[@]:1}"; do
            ...
            time /HFDGF-slicer/build/bin/slicer --targets=./targets.txt ./$BIN_NAME.bc > /sa_log/$1-$BIN_NAME-$BUG_NAME.txt
            cd ..

            step2_end_time=$(date +%s)
            step2_duration=$((step2_end_time-step2_start_time))
            last_line=$(tail -n 1 /sa_log/$1-$BIN_NAME-$BUG_NAME.txt)
            echo "HFDGF,$1,$BIN_NAME,$BUG_NAME,$step1_start_time,$step1_end_time,$step1_duration,$step2_start_time,$step2_end_time,$step2_duration,$last_line" >> /sa_time_bug.txt

            rm -rf $HFDGF_WORKDIR
            # mv $HFDGF_WORKDIR /benchmark/HFDGF_pre_slicer/RUNDIR-$1-$BIN_NAME-$BUG_NAME
        done
    done
...
rm -rf /benchmark/HFDGF_pre_slicer
mkdir -p /benchmark/HFDGF_pre_slicer
rm -rf /sa_log
mkdir /sa_log
