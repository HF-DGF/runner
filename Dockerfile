...
# copy target projects source to /projects_source
WORKDIR /
COPY docker-setup/projects_source.tar.gz /projects_source.tar.gz
RUN tar zxf projects_source.tar.gz && rm -rf projects_source.tar.gz
...
#### Setup DAFL.
WORKDIR /fuzzer
COPY docker-setup/setup_HFDGF.sh /fuzzer/setup_HFDGF.sh
RUN ./setup_HFDGF.sh

#### Build benchmark with HFDGF.
COPY docker-setup/build_bench_HFDGF.sh /benchmark/build_bench_HFDGF.sh
RUN ./build_bench_HFDGF.sh
